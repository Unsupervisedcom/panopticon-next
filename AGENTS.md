...
- **Breaking releases require explicit user approval.** Agents MUST NOT make or merge a
  ...
  write a `type!:` commit header or `BREAKING CHANGE:` footer, or merge a Release Please PR that
  proposes a major version, without that approval. Before merging any Release Please PR, inspect
  its proposed version; a major bump is a stop-and-ask.
