...
1. On its native `input` event, the Pi harness MUST await settlement of the task-service turn
   write request before that event handler finishes, selecting `agent` during an active run or
   when a model and configured auth are present, otherwise `user`, subject to REQ-016's bounded
   fail-open contract.
