...
printf 'runtime:%s|base_image=%s|port=%s|service=%s|container_service=%s|tmux=%s|docker_host=%s|docker_config=%s|docker_context=%s\\n' \\
  "${PANOPTICON_RUNTIME_ID-unset}" "${PANOPTICON_BASE_IMAGE-unset}" \\
  "${PANOPTICON_PORT-unset}" "${PANOPTICON_SERVICE_URL-unset}" \\
  "${PANOPTICON_CONTAINER_SERVICE_URL-unset}" \\
  "${TMUX_TMPDIR-unset}" "${DOCKER_HOST-unset}" "${DOCKER_CONFIG-unset}" \\
...
""",
    ...
    _executable(
        fake_bin / "python3",
        'case "$*" in *getsockname*) printf \'41873\\n\' ;; *) exec "$REAL_PYTHON" "$@" ;; esac\n',
    )
    _executable(
        fake_bin / "uv",
        """
...
def test_dev_tmp_home_reports_host_auth_without_copying_it(tmp_path: Path) -> None:
    ...
    assert (
        f"|docker_host=unix:///tmp/fake-docker.sock|docker_config={env['HOME']}/.docker"
        "|docker_context=unset" in observed
    )
    assert "panopticon:stop" in observed


def test_dev_tmp_home_copies_codex_auth_only_with_explicit_opt_in(tmp_path: Path) -> None:
    env, _, _ = _fake_environment(tmp_path)
    env["PANOPTICON_DEV_TMP_HOME_AUTH"] = "1"

    completed = subprocess.run(
        [str(DEV)], env=env, cwd=ROOT, capture_output=True, text=True, check=True
    )

    home = Path(completed.stdout.partition("Panopticon dev HOME: ")[2].splitlines()[0])
    auth = home / ".codex" / "auth.json"
    assert auth.read_text() == '{"auth_mode":"chatgpt"}'
    assert stat.S_IMODE(auth.stat().st_mode) == 0o600
    assert "WARNING: copying a rotating Codex login" in completed.stdout
